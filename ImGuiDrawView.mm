...
// ── Colors ──────────────────────────────────────────────────────────────────
static const ImU32 ZX_WIN_BG        = IM_COL32( 30,  30,  32, 248);   // near-black bg
static const ImU32 ZX_TITLE_BG      = IM_COL32( 30,  30,  32, 255);   // same as bg
static const ImU32 ZX_PANEL_BG      = IM_COL32( 30,  30,  32, 255);
static const ImU32 ZX_PANEL_BORDER  = IM_COL32( 60,  60,  63, 255);
static const ImU32 ZX_SIDE_BTN_BG   = IM_COL32( 50,  50,  52, 255);   // tab button
static const ImU32 ZX_SIDE_BTN_ACT  = IM_COL32( 70,  70,  73, 255);   // active tab
static const ImU32 ZX_SIDE_BORDER   = IM_COL32( 55,  55,  58, 255);
static const ImU32 ZX_SIDE_BORDER_A = IM_COL32( 80,  80,  83, 255);
static const ImU32 ZX_TAB_TEXT      = IM_COL32(255, 255, 255, 255);
static const ImU32 ZX_TAB_TEXT_DIM  = IM_COL32(170, 170, 175, 255);
static const ImU32 ZX_TAB_UNDERLINE = IM_COL32(255, 255, 255, 200);
static const ImU32 ZX_TAB_DIV       = IM_COL32( 55,  55,  58, 200);
static const ImU32 ZX_SEP           = IM_COL32( 55,  55,  58, 200);
...
// iOS toggle colors (replaces checkbox)
static const ImU32 ZX_TGL_ON        = IM_COL32( 52, 199,  89, 255);   // iOS green
static const ImU32 ZX_TGL_OFF       = IM_COL32( 99,  99, 102, 255);   // iOS gray
static const ImU32 ZX_TGL_KNOB      = IM_COL32(255, 255, 255, 255);
static const ImU32 ZX_HOVER         = IM_COL32(255, 255, 255,  12);
// keep slider colors neutral
static const ImU32 ZX_CHK_BG        = IM_COL32( 50,  50,  52, 255);
static const ImU32 ZX_CHK_BG_ON     = IM_COL32( 52, 199,  89, 255);
static const ImU32 ZX_CHK_BORDER    = IM_COL32( 80,  80,  83, 255);
static const ImU32 ZX_CHK_BORDER_ON = IM_COL32( 52, 199,  89, 255);
static const ImU32 ZX_CORNER_YELLOW = IM_COL32(255, 255, 255, 200);
static const ImU32 ZX_SLIDER_BG     = IM_COL32( 50,  50,  52, 255);
...
// ── Layout ──────────────────────────────────────────────────────────────────
static const float ZX_WIN_W      = 380.0f;
static const float ZX_WIN_H      = 470.0f;
static const float ZX_TITLE_H    = 46.0f;   // title bar
static const float ZX_TOP_PAD    = 0.0f;
static const float ZX_TAB_H      = 0.0f;    // ไม่มี top tab bar แล้ว
static const float ZX_SIDE_W     = 95.0f;   // sidebar กว้าง
static const float ZX_BOT_H      = 54.0f;   // Close/HIDE bar
static const float ZX_SIDE_BTN   = 46.0f;
static const float ZX_SIDE_GAP   = 8.0f;
static const float ZX_ROW_H      = 50.0f;   // ความสูงแต่ละแถว toggle
static const float ZX_SLIDER_H   = 22.0f;
static const float ZX_DROP_H     = 24.0f;
...
static const float ZX_CHK_BOX    = 18.0f;
static const float ZX_CHK_RAD    = 5.0f;
static const float ZX_KNOB_R     = 6.0f;
static const float ZX_WIN_RAD    = 12.0f;
static const float ZX_FRAME_RAD  = 6.0f;
...
static bool ZX_SonicCheckCell(ImVec2 cellMin, ImVec2 cellMax, const char* label, bool* v) {
    ...
    // เส้นแบ่งแถว
    dl->AddLine(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Min.y), ZX_SEP, 0.5f);
...
static void RenderMenu() {
    ...
    for (int i = 0; i < kTabs; ++i) {
        ...
        dl->AddText(ImVec2(wp.x + (ZX_SIDE_W - ts.x) * 0.5f,
                           ty0 + (tabH - ts.y) * 0.5f),
                    tc, tabNames[i]);
