...
struct Vars_t
{
    ...
    const char *dir[4] = {"None", "Fire", "Scope", "BắnและNgắm"};
...
void DrawSkeleton(void *player, ImDrawList *drawList)
{
    ...
    // Chuyển đổi vị trí xương sang tọa độ màn形
    bool visible;
    ...
    if (SilentAim && HitObject) {
        ...
        if (match) {
            ...
            if (localPlayer) {
                ...
                void *enemy = GetClosestEnemy(); // แก้ไขเป็น GetClosestEnemy เพราะในไฟล์เดิมน่าจะเป็นฟังก์ชันหาศัตรู
                if (enemy && weapon) {
...
void get_players()
{
    ...
    try
    {
        ...
        for (int u = 0; u < players->getSize(); u++)
        {
            ...
            // แก้ไข ESP BOX สีชมพู
            ImColor line_color = ImColor(255, 105, 180); // Pink (Hot Pink)
            bool w2sc;
            ...
            if (w2sc)
            {
                if (Vars.lines)
                {
                    if (game_sdk->get_IsDieing(closestEnemy))
                    {
                        draw_list->AddLine(ImVec2(ImGui::GetIO().DisplaySize.x / 2, ImGui::GetIO().DisplaySize.y), ImVec2(rect.GetCenter().x, rect.Max.y), ImColor(255, 0, 0), 1.0f);
                    }
                    else
                    {
                        // แก้ไข ESP LINE เป็นเส้นประอยู่ล่างจอ
                        draw_list->PushClipRectFullScreen();
                        for (float i = 0; i < 1.0f; i += 0.02f) {
                            ImVec2 start = ImVec2(ImGui::GetIO().DisplaySize.x / 2 + (rect.GetCenter().x - ImGui::GetIO().DisplaySize.x / 2) * i, ImGui::GetIO().DisplaySize.y + (rect.Max.y - ImGui::GetIO().DisplaySize.y) * i);
                            ImVec2 end = ImVec2(ImGui::GetIO().DisplaySize.x / 2 + (rect.GetCenter().x - ImGui::GetIO().DisplaySize.x / 2) * (i + 0.01f), ImGui::GetIO().DisplaySize.y + (rect.Max.y - ImGui::GetIO().DisplaySize.y) * (i + 0.01f));
                            draw_list->AddLine(start, end, line_color, 1.0f);
                        }
                ...
                if (Vars.Box)
                {
                    ...
                    {
                        // แก้ไข ESP BOX เป็นสีชมพู
                        draw_list->AddRect(rect.Min, rect.Max, line_color);
                    }
                ...
                if (Vars.Name)
                {
                    ...
                    // แก้ไข ESP NAME เป็นสีดำ
                    AddText(verdana_smol, 8, false, Vars.Outline, name_pos, ImColor(0, 0, 0), name);
                }
                if (Vars.Health)
                {
...
void aimbot()
{
    ...
    if (Vars.isAimFov)
    {
        if (Vars.fovaimglow)
            ...
            // แก้ไข FOV เป็นเส้นประ
            float radius = Vars.AimFov;
            int num_segments = 100;
            for (int i = 0; i < num_segments; i += 2) {
                float a1 = ((float)i / (float)num_segments) * (2.0f * M_PI);
                float a2 = ((float)(i + 1) / (float)num_segments) * (2.0f * M_PI);
                draw_list->AddLine(
                    ImVec2(center.x + cosf(a1) * radius, center.y + sinf(a1) * radius),
                    ImVec2(center.x + cosf(a2) * radius, center.y + sinf(a2) * radius),
                    ImColor(Vars.fovLineColor[0], Vars.fovLineColor[1], Vars.fovLineColor[2], Vars.fovLineColor[3]),
                    1.0f
                );
            }
        }
